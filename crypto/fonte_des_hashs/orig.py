...
# 256 bytes
# len sbox == len set sbox > True 
# means that sbox values are all unique
# thus, the index can be reversed
...
# converts chars to bitstrings (if not ascii, create 8bit segments of the UTF-8 char)
# pads it up to a multiple of 32
# if size > 32 (any string with more than 32 chars), xors n with n+1 for every 32 bit block
# xors 3 times binary with itself

def h(m):
    plain = m
    binary = string2bits(plain)
    padding(binary)
    print(f"binary after padding : {binary}")
    if len(binary) > 32:
        binary = phase1(binary)
        print(f"binary after p1 : {binary}")
    phase2(binary)
    print(f"binary after p2_1 : {binary}")
    phase2(binary)
    print(f"binary after p2_2 : {binary}")
    phase2(binary)
    print(f"binary after p2_3 : {binary}")
    sbox_ope(binary)
    print(f"binary after sbox : {binary}")
