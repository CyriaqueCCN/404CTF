...
# tab[256] of bytes. Uniques values, so indexes can be reversed
...
def rev_p1(binary):
    # phase_1 is destructive.
    # as such, we can't recover input if it's more than 32 chars
    # (including padding) which means more than 30 chars unpadded
    pass # so we hope for the flag to be less than that
...
def hex2bits(s):
    ...
    for i in range(0, len(s), 2):
        b.append(int2bits(int(s[i:i+2], 16)))
    return b


def unpad(binary):
    i = len(binary) - 1
    while i >= 0 and binary[i] in ["00000001", "00000000"]:
        binary.pop(i)
        i -= 1
...
def rev_p2(binary):
    ...
    for i in range(1, len(binary)):
        for j in range(i):
            binary[i] = xor(binary[i], b[j])


def rev_sbox(binary):
    for i in range(len(binary)):
        binary[i] = int2bits(sbox.index(binary[i]))


def reverse(m):
    binary = hex2bits(m)
    rev_sbox(binary)
    if DEBUG:   print(f"binary after rev_sbox : {binary}")
    rev_p2(binary)
    if DEBUG:   print(f"binary after p2_3 : {binary}")
    rev_p2(binary)
    if DEBUG:   print(f"binary after p2_2 : {binary}")
    rev_p2(binary)
    if DEBUG:   print(f"binary after p2_1 : {binary}")
    rev_p1(binary)
    unpad(binary)
    return bits2string(binary)


def main():
    cipher = ""
    if len(sys.argv) < 2:
        print("Aucun argument donné. Rien à reverse. Bye bye.")
        exit()
    cipher += sys.argv[1]
    for i in range(2, len(sys.argv)):
        cipher += " " + str(sys.argv[i])
